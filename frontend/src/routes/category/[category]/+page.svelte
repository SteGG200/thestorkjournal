...
<div class="min-h-screen mx-auto w-3/4 max-xl:w-4/5 max-md:w-full max-md:px-4 pt-4 flex max-md:flex-col-reverse justify-center max-md:justify-end md:space-x-4">
		...
		<div class="w-2/3 max-md:w-full space-y-8">
			<div class="flex flex-col space-y-4 border-b-2 pb-8 w-5/6 max-[1100px]:w-full">
				<p class="text-3xl max-sm:text-2xl font-bold">
					<a href={`/${data.articles[0].id}`} class="hover:text-slate-600">{data.articles[0].title}</a>
				</p>
				<p class="text-gray-500">
					By {data.articles[0].name} on {new Date(data.articles[0].date_publish).toLocaleDateString('en-US',{
						year: 'numeric', 
						month: 'long',
						day: 'numeric'
					})}
				...
				<a href={`/${data.articles[0].id}`}>
					<img class="aspect-[14/8] w-full object-cover hover:brightness-110" src={data.articles[0].thumbnail} alt="notfound">
				</a>
	...
	{:else}
		<p class="text-4xl text-center">Sorry, we can't find any article related to '{formatCategory($page.params.category)}' for you.</p>
	{/if}
</div>
