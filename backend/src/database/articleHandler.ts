...
export const getListArticles = async (category: string = '') => {
	if (category != '') {
		return await sql<
			ArticleInfoSchema[]
		>`SELECT date_publish, title, thumbnail, category, content FROM articles 
...
export const getArticleById = async (articleId: string) => {
	return await sql<
		ArticleInfoSchema[]
	>`SELECT date_publish, user_id, title, thumbnail, category, content FROM articles WHERE id = ${articleId}`;
};
